...
export const UserSchema = {
  ...
  fields: {
    ...
    twitterSubscription: "String (None, Basic, Premium, PremiumPlus)",
    verifiedType: "String (none, blue, government, business)",
...
export const updateUserCredits = async (db, userId, creditsUsed) => {
  ...
  return await users.updateOne(
    ...
    { $inc: { credits: -creditsUsed }, $set: { updatedAt: new Date() } },
  )
}
  ...
  return await users.updateOne(
    { _id: new ObjectId(userId) },
    {
      $set: {
        ...
        twitterSubscription: twitterData.subscription_type || "None",
        verifiedType: twitterData.verified_type || "none",
...
export const isTwitterTokenExpired = (user) => {
  if (!user.twitterTokenExpiresAt) return true
  return new Date() >= new Date(user.twitterTokenExpiresAt)
}
